...
if sys.version_info[:2] >= (3, 4):
    ...
    def _RedirectOutput(new_target):
        with open(new_target, 'a') as fp:
  ...
  class _RedirectOutput(object):
    ...
    def __enter__(self):
        ...
        if isinstance(self._new_target, six.string_types):
...
class Config(collections.MutableMapping):
    ...
    def prompt(self, config_translation, args=None, defaults={}):
        ...
        for (argkey, confkey) in config_translation:
            value = self.get(confkey, None)

            if not value and args:
                value = getattr(args, argkey, None)
            ...
            if value is None:
                ...
                if default is not None:
                    ...
                    value = six.moves.input("%s = " % confkey).strip()
            else:
...
def sudo_with_input(data, *args):
    with tempfile.TemporaryFile('w+t') as inputfp:
        if data:
            inputfp.write(data)
            inputfp.seek(0)
...
def create_user(user, group='mox'):
    try:
        grp.getgrnam(group)
        print('Using pre-existing group {!r}'.format(user))
    except KeyError:
        print('Creating group {!r}'.format(user))
        sudo(
